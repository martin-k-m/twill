...
| [tutorial-systems.md](tutorial-systems.md) | The other half: `mode systems`, integers, strings, arrays, structs, enums, `?`, files, ending in a working expression parser. |
...
Claims about correctness and speed, with the measurements behind them. Every
number is regenerated by a script in `bench/`.

| Document | What it is |
| --- | --- |
| [CORRECTNESS.md](CORRECTNESS.md) | Gradient checking over the full operator set, and what the shape checker does and does not guarantee. Names the operator that failed and what it cost. |
| [BENCHMARKS.md](BENCHMARKS.md) | twill against PyTorch on the same mathematics, with the environment, the method, medians and p99s, and a profile of where the time actually goes. |
| [BUGS.md](BUGS.md) | The defects found so far, each with its mechanism, the commit that fixed it and the test that catches it coming back. |
...
| [DECISIONS.md](DECISIONS.md) | Eight choices that could have gone the other way: the alternative that was considered and why it lost, including what each one costs. |
| [CODEGEN.md](CODEGEN.md) | The design for compiling twill rather than interpreting it: a tracing IR, the fusion strategy, what compiles and what does not, and the benchmark that would prove it worked. Design only, no code. |
...
under `src/`, and as of v1.4.0 it runs on that reference and is compared against
it stage by stage. These four documents are that effort.
...
1. [tutorial.md](tutorial.md), to get a program running, and
   [tutorial-systems.md](tutorial-systems.md) for the half of the language that
   is not about tensors.
