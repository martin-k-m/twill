...
// Canonical dump mode. `raster run <file> --dump=canonical` runs a program and
...
func runFileCanonical(path string, check bool) int {
	...
	if dir := filepath.Dir(path); dir != "" {
		if err := os.Chdir(dir); err != nil {
			fmt.Fprintf(os.Stderr, "raster: cannot enter %q\n", dir)
			return 1
		}
	}
	...
	// The program's final value comes first, under a name no Raster program can
...
// way on purpose: in Raster they are the same value, and which of the two
...
// cannot distinguish two NaN payloads; no Raster operation makes that
